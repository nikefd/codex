...
pub fn flush_auth_failure_event_queue(timeout: Duration) {
    ...
    std::thread::scope(|scope| {
        scope.spawn(|| {
            flush_auth_failure_upload_queue(auth_failure_upload_queue_sender(), "primary", deadline)
        });
        scope.spawn(|| {
            flush_auth_failure_upload_queue(
                fallback_auth_failure_upload_queue_sender(),
                "fallback",
                deadline,
            )
...
pub fn flush_auth_failure_event_queue_and_exit_failure(timeout: Duration) -> ! {
    flush_auth_failure_event_queue_and_exit(timeout, /*code*/ 1)
}
...
fn enqueue_auth_failure_upload(
    ...
    match sender.try_send(AuthFailureUploadCommand::Upload(task)) {
        Ok(()) => true,
        Err(err) => {
...
fn auth_failure_upload_queue_sender_for(
    slot: &'static OnceLock<std::sync::mpsc::SyncSender<AuthFailureUploadCommand>>,
    capacity: usize,
    lane: &'static str,
) -> &'static std::sync::mpsc::SyncSender<AuthFailureUploadCommand> {
    slot.get_or_init(|| {
...
fn flush_auth_failure_upload_queue(
    sender: &std::sync::mpsc::SyncSender<AuthFailureUploadCommand>,
    lane: &'static str,
    deadline: std::time::Instant,
) {
    let (tx, rx) = std::sync::mpsc::channel();
    if let Err(err) = send_auth_failure_flush_command(sender, tx, deadline) {
        tracing::warn!(error = %err, lane, "failed to request auth failure queue flush");
        return;
    }
    let remaining = deadline.saturating_duration_since(std::time::Instant::now());
    if let Err(err) = rx.recv_timeout(remaining) {
        tracing::warn!(error = %err, lane, "timed out flushing auth failure queue");
    }
}
